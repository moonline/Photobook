import ImageController = require("Application/ImageController");
import PPIService = require("../Service/PPIService");
import AngularRoute = require("../Service/PPIService");

class ImageModule {
	app:any;

	constructor(require:any, Angular: any) {
		this.app = Angular.module('App', [ 'ngRoute' ]);

		this.setConfig(require);
		this.addDirectives();
		this.addControllers();
	}
	...
	private addDirectives() {
		this.app.directive('contenteditable', function() {
			return {
				require: 'ngModel',
		 ...
		 */
		this.app.directive('dragpoint', function($document, $parse) {
		...
		this.app.directive('ngChange', function() {
			return {
				//restrict: 'A',
				scope:{'ngChange':'=' },
		 ...
		 */
		this.app.directive('ngInput', function($document,$parse) {
			return {
				scope: {
					callback: '&callback'
				},
	...
	private addControllers() {
		this.app.controller("ImageController", ["$scope", "$location", ($scope, $location)
			=> new ImageController($scope, $location)]);
	}
}
